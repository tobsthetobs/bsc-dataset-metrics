...
def load_aqualoc_dataset(supress_output: bool):
    ...
    for subfolder in data:
        ...
        for file in os.listdir(os.path.join(dir,subfolder)):
            ...
            # Check number of image channels, convert to HSV if not a grayscale image.
            if check_image_dim(image):
                im_hsv = load_to_colorspace(image, "HSV")
                sequence.append(np.mean(im_hsv))
            else:
                sequence.append(np.mean(image))
            
        data_brightness.append(sequence)
    return data_brightness
...
def create_histogram_aqualoc(data_tuple):
